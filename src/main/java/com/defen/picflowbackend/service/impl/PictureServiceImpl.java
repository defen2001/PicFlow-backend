...
import com.defen.picflowbackend.api.aliyunai.AliYunAiApi;
import com.defen.picflowbackend.api.aliyunai.model.CreateOutPaintingTaskRequest;
import com.defen.picflowbackend.api.aliyunai.model.CreateOutPaintingTaskResponse;
        ...
        transactionTemplate.execute(status -> {
            ...
            if (spaceId1 != null) {
                ...
                boolean update = spaceService.lambdaUpdate()
                        ...
                        .update();
                ExceptionUtils.throwIf(!update, ErrorCode.OPERATION_ERROR, "额度更新失败");
            }
            return picture;
        });
        //todo 更新需要清理图片资源
...
//        String cachedValue = cacheManager.getCache(localKey, redisKey);
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ExceptionUtils.throwIf(count > 30, ErrorCode.PARAM_ERROR, "最多 30 条");
        // 要抓取的地址
...
//        Elements imgElementList = div.select("img.mimg"); // 获取缩略图
        Elements imgElementList = div.select(".iusc"); // 获取高清图（包含完整数据的元素）
        int uploadCount= 0;
        // 遍历元素，依次处理上传图片
...
//            String fileUrl = imgElement.attr("src");
            ...
            // 获取高清图模式
            String dataM = imgElement.attr("m"); // 获取data-m属性中的JSON字符串
            String fileUrl;
            try {
                // 解析JSON字符串
    ...
    public void deletePicture(long pictureId, User loginUser) {
        ExceptionUtils.throwIf(pictureId <= 0, ErrorCode.PARAM_ERROR);
        ExceptionUtils.throwIf(loginUser == null, ErrorCode.UNAUTHORIZED_ERROR);
        // 判断是否存在
        Picture oldPicture = this.getById(pictureId);
        ExceptionUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR);
        // 校验权限
    ...
    public void editPictureByBatch(PictureEditByBatchRequest pictureEditByBatchRequest, User loginUser) {
        ...
        // 1.校验参数
        ExceptionUtils.throwIf(spaceId == null || CollUtil.isEmpty(pictureIdList), ErrorCode.PARAM_ERROR);
        ExceptionUtils.throwIf(loginUser == null, ErrorCode.UNAUTHORIZED_ERROR);
        // 2.校验空间权限
        Space space = spaceService.getById(spaceId);
        ExceptionUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
        if (!loginUser.getId().equals(space.getUserId())) {
            throw new BusinessException(ErrorCode.UNAUTHORIZED_ERROR, "没有空间访问权限");
        }
        // 3.查询指定图片，仅选择需要的字段
